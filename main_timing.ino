...
void loop() {
    ...
    if (currentMillis - previousMillis >= (LOOP_TIME * 1000)) {
        ...
        unsigned long loop_start = micros(); // 전체 루프 시작 시간
        ...
        unsigned long front_time = micros() - sensor_start;
        
        unsigned long left_start = micros();
        float newLeft  = readDistance(TRIG_LEFT, ECHO_LEFT);
        unsigned long left_time = micros() - left_start;
        
        unsigned long right_start = micros();
        float newRight = readDistance(TRIG_RIGHT, ECHO_RIGHT);
        unsigned long right_time = micros() - right_start;
        
        unsigned long total_sensor_time = micros() - sensor_start;
        ...
        // 2. SMC 제어 계산
        // ----------------------------------
        unsigned long smc_start = micros();
        
        float uFront = smcFront.update(CELL_DISTANCE, lastFront);
        unsigned long front_smc_time = micros() - smc_start;
        
        unsigned long lat_smc_start = micros();
        float errorLat = lastLeft - lastRight; 
        float uLat = smcLat.update(0, errorLat);
        unsigned long lat_smc_time = micros() - lat_smc_start;
        
        unsigned long total_smc_time = micros() - smc_start;


        // ----------------------------------
        // 3. 모터 믹싱
        // ----------------------------------
        unsigned long mixing_start = micros();
        ...
        unsigned long total_mixing_time = micros() - mixing_start;


        // ----------------------------------
        // 4. 전방 벽 감지 및 주행 
        // ----------------------------------
        unsigned long motor_start = micros();
        ...
        unsigned long total_motor_time = micros() - motor_start;
        
        // ----------------------------------
        // 5. 미로 탐색 종료 조건 체크
        // ----------------------------------
        unsigned long exit_start = micros();
        ...
        unsigned long total_exit_time = micros() - exit_start;


        // ----------------------------------
        // 6. 시리얼 로깅
        // ----------------------------------
        unsigned long serial_start = micros();
        ...
        unsigned long total_serial_time = micros() - serial_start;
        
        
        // ----------------------------------
        // 7. 전체 루프 시간
        // ----------------------------------
        unsigned long total_loop_time = micros() - loop_start;


        // ----------------------------------
        // 8. 타이밍 정보 출력 (매 20번마다)
        // ----------------------------------
        static unsigned long timing_counter = 0;
        if (timing_counter++ % 20 == 0) {
            Serial.println();
            Serial.println(F("========== TIMING ANALYSIS =========="));
            Serial.print(F("1. Sensor Total: "));
            Serial.print(total_sensor_time / 1000.0, 2);
            Serial.println(F(" ms"));
            Serial.print(F("   ├─ Front: "));
            Serial.print(front_time / 1000.0, 2);
            Serial.println(F(" ms"));
            Serial.print(F("   ├─ Left: "));
            Serial.print(left_time / 1000.0, 2);
            Serial.println(F(" ms"));
            Serial.print(F("   └─ Right: "));
            Serial.print(right_time / 1000.0, 2);
            Serial.println(F(" ms"));
            
            Serial.print(F("2. SMC Total: "));
            Serial.print(total_smc_time / 1000.0, 2);
            Serial.println(F(" ms"));
            Serial.print(F("   ├─ Front SMC: "));
            Serial.print(front_smc_time / 1000.0, 2);
            Serial.println(F(" ms"));
            Serial.print(F("   └─ Lateral SMC: "));
            Serial.print(lat_smc_time / 1000.0, 2);
            Serial.println(F(" ms"));
            
            Serial.print(F("3. Motor Mixing: "));
            Serial.print(total_mixing_time / 1000.0, 2);
            Serial.println(F(" ms"));
            
            Serial.print(F("4. Motor Control: "));
            Serial.print(total_motor_time / 1000.0, 2);
            Serial.println(F(" ms"));
            
            Serial.print(F("5. Exit Check: "));
            Serial.print(total_exit_time / 1000.0, 2);
            Serial.println(F(" ms"));
            
            Serial.print(F("6. Serial Output: "));
            Serial.print(total_serial_time / 1000.0, 2);
            Serial.println(F(" ms"));
            
            Serial.print(F("\n[TOTAL LOOP TIME]: "));
            Serial.print(total_loop_time / 1000.0, 2);
            Serial.println(F(" ms"));
            
            Serial.print(F("[TARGET LOOP TIME]: "));
            Serial.print(LOOP_TIME * 1000, 2);
            Serial.println(F(" ms"));
            
            if (total_loop_time / 1000.0 > LOOP_TIME * 1000) {
                Serial.println(F("WARNING: Loop overrun!"));
            } else {
                Serial.println(F("Loop timing OK"));
            }
...
// 보조 함수 구현
// ==========================================
// 초음파 센서 거리 측정 함수 (타임아웃 8000us로 변경)
float readDistance(int trigPin, int echoPin) {
    digitalWrite(trigPin, LOW); delayMicroseconds(2);
    digitalWrite(trigPin, HIGH); delayMicroseconds(10);
    digitalWrite(trigPin, LOW);
    
    long duration = pulseIn(echoPin, HIGH, 8000); // 20000 → 8000으로 변경
